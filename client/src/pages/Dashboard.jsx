...
import { generateEmail, clearError } from "../redux/aiSlice";

const Dashboard = () => {
  const dispatch = useDispatch();

  const { loading, error, result } = useSelector((state) => state.ai);
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      await dispatch(generateEmail(prompt)).unwrap();
      ...
      <div className="w-full lg:w-1/3 bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
        ...
        <form onSubmit={handleGenerate} className="flex-1 flex flex-col">
          ...
          <textarea
            ...
            placeholder="e.g. Write a cold email to a marketing director at a SaaS company..."
          />
